...
- `e2e_gate` — `y` | `n`
- `dev_policy` — `single-tenant` | `per-branch-preview`
...
| 6 | **Dev deploy** | `git push origin <branch>`. Trigger CI (`gh workflow run build.yml --ref <branch>` then `deploy.yml`). Poll `{{DEV_DOMAIN}}/health` with backoff (max 10 min). `single-tenant`: never run two `/run-issue` concurrently. `per-branch-preview`: deploys a unique preview per branch — concurrency safe. |
...
| Phase | Action |
|---|---|
| Deploy-check | Verify each touched repo's dev state matches the branch HEAD (or `dev` HEAD if merged). Mismatch → ask user per-repo: deploy / skip / abort. |
| APK rebuild | Only if mobile repo touched and APK stale. Uses `envied` `.env` flow (no flavors). |
| Appium E2E | Run full suite against dev. Failure → **E2E Triage** (budget: `{{iter_budget_run_e2e}}`). Findings post as PR comments; PRs stay draft until gate passes. |
...
| 4 | Dev deploy | `gh workflow run` deploys feature branch; CI polls `/health` |
...
- Never run two `/run-issue` concurrently when `dev_policy = single-tenant`
