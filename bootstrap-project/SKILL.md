...
3. **Code repos** (per `microservices.md`)
   ...
   - `<project>-web/` — Next.js 15 + React 19 public web skeleton (if web ≠ none). Stamps:
     ...
     - Env shape: `AUTH_GOOGLE_ID` (public — used by the browser to initialize GIS), `AUTH_SECRET`, `AUTH_TRUST_HOST=true`, `{{PROJECT_UPPER}}_API_BASE_URL` (defaults to `http://nginx/api` in docker-network dev so the in-cluster nginx fronts user-service + `<project>-core` from a single URL). **No `AUTH_GOOGLE_SECRET`** — the OAuth client secret lives only in user-service.
