 ...
 * server-side signIn callback) to user-friendly copy.
 */
function describeError(code: string): string {
  switch (code) {
    ...
    case 'AccessDenied':
      return 'Access denied. Your Google account is not authorised for this app.';
...
export function LoginForm({ error, nextPath }: LoginFormProps) {
  ...
  async function handleGoogleClick() {
    setPending(true);
    try {
      await signIn('google', { callbackUrl: nextPath ?? '/dashboard' });
    } finally {
  ...
  return (
    <div className="space-y-4">
      ...
      <button
        ...
        disabled={pending}
        className="flex w-full items-center justify-center gap-3 rounded-md border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-900 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
      >
